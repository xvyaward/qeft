...
#define KERNEL_LAUNCH_CODE                                                                                                                              \
  ...
  auto kernel_func = gemm_w4a16_T1_qeft<CTA_M, CTA_N, CTA_K, WARP_M, WARP_N, WARP_K, STAGES, G, SPLITK>;                                                     \
  ...
  kernel_func<<<num_blocks, threads_per_block, kSmemByteSize>>>(                                                                                        \
      in_feats, kernel, scales, zeros, oweights, out_feats, semaphores, num_in_feats, num_out_channels, num_in_channels);
...
__device__ __inline__ void global_to_share_one_stage_A(half *src, half *dst, int global_nrows, int global_ncols, int cta_offset_m, int cta_offset_n, int cta_offset_k, int global_iter_k, int shared_iter_k, bool mask)
{
  ...
  constexpr int threads_per_row = CTA_K / PACK_SIZE; // 16
  ...
  //   printf("\nthreads_needed : %d threads_used : %d total_global_iters : %d partial_global_iters : %d cta_step_m_or_n : %d warp_step_m_or_n : %d threads_per_row : %d kSmemCol : %d ld_col : %d\n",threads_needed, threads_used, total_global_iters, partial_global_iters, cta_step_m_or_n, warp_step_m_or_n, threads_per_row, kSmemCol, ld_col);
  // }
#pragma unroll
  for (int _global_iter = 0; _global_iter < partial_global_iters; ++_global_iter)
  {
    ...
    // if (blockIdx.x == 0 && threadIdx.x == 0 && threadIdx.y == 0){
    //   printf("global_iter : %d ld_row : %d ld_col_swizzled : %d dst_ptr_offset : %d src_ptr_offset %d\n", global_iter, ld_row, ld_col_swizzled, ld_row * kSmemCol + ld_col_swizzled, (ld_row + cta_offset_m) * global_ncols + ld_col * PACK_SIZE + global_iter_k * CTA_K + cta_offset_k);
    // }
...
template <int CTA_M, int CTA_N, int CTA_K, int CTA_SIZE, int SHARED_K_ITERS, int STAGES>
__device__ __inline__ void global_to_share_one_stage_B_half(half *src, half *dst, int global_ncols, int cta_offset_m, int cta_offset_n, int cta_offset_k, int global_iter_k, int shared_iter_k, bool mask)
{
  constexpr int threads_needed = (CTA_N / kInterleave * CTA_K) / PACK_SIZE / SHARED_K_ITERS;
  constexpr int threads_used = threads_needed < CTA_SIZE ? threads_needed : CTA_SIZE;
  constexpr int total_global_iters = (CTA_N / kInterleave * CTA_K) / PACK_SIZE / threads_used;
...
__global__ void gemm_w4a16_T1_qeft(half *__restrict__ A, half *__restrict__ B, half *__restrict__ scales, half *__restrict__ zeros, half *__restrict__ B_half, half *__restrict__ C, int *__restrict__ semaphores, int M, int N, int K)
{
...
#pragma unroll
  ...
  if (blockIdx.x == 0 && threadIdx.x == 0 && threadIdx.y == 0){
    printf("NUM_WARPS_MN : %d, NUM_WARPS : %d, CTA_SIZE : %d, CTA_SIZE_MN : %d, SLICES : %d, num_blocks_n : %d, num_blocks_m : %d, blockIdx_y : %d, blockIdx_z : %d, blockIdx_m : %d, blockIdx_n : %d, block_idx_mapping.x : %d, block_idx_mapping.y : %d\n", NUM_WARPS_MN, NUM_WARPS, CTA_SIZE, CTA_SIZE_MN, SLICES, num_blocks_n, num_blocks_m, blockIdx_y, blockIdx_z, blockIdx_y / (num_blocks_n >> log_tile), blockIdx_y % (num_blocks_n >> log_tile), block_idx_mapping.x, block_idx_mapping.y);
    printf("kSmemPadKA : %d, kSmemPadKB : %d, kSmemSizeAPerStage : %d, kSmemSizeBPerStage : %d, kSmemSizeA : %d, kSmemSizeB : %d, scales_load_interval : %d, scales_per_load : %d, kSmemSizeScales : %d, kSmemSizeZeros : %d\n", kSmemPadKA, kSmemPadKB, kSmemSizeAPerStage, kSmemSizeBPerStage, kSmemSizeA, kSmemSizeB, scales_load_interval, scales_per_load, kSmemSizeScales, kSmemSizeZeros);
    printf("cta_offset_m : %d, cta_offset_n : %d, cta_offset_k : %d, warp_mn : %d, slice_id : %d, warp_offset_n : %d, warp_offset_m : %d, warp_offset_k : %d, gemm_iters : %d, prologue_stages : %d, SHARED_K_ITERS : %d\n", cta_offset_m, cta_offset_n, cta_offset_k, warp_mn, slice_id, warp_offset_n, warp_offset_m, warp_offset_k, gemm_iters, prologue_stages, SHARED_K_ITERS);
  }
  // half *A_shared_warp_ptr = A_shared_warp_[0];
  // half *B_shared_warp_ptr = B_shared_warp_[0];
  // if (blockIdx.x == 0 && threadIdx.x == 0 && threadIdx.y == 0){
  //   printf("gridDim.x : %d, blockDim.x : %d, blockDim.y : %d\n", gridDim.x, blockDim.x, blockDim.y);
    // float printf_buffer[64];
    // for (int i=0; i < 64; ++i){
    //   printf_buffer[i] = __half2float(*(B_shared_warp_ptr+i));
    // }
    // printf("B_shared_warp_[:32] : \n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n\n", printf_buffer[0], printf_buffer[1], printf_buffer[2], printf_buffer[3], printf_buffer[4], printf_buffer[5], printf_buffer[6], printf_buffer[7], printf_buffer[8], printf_buffer[9], printf_buffer[10], printf_buffer[11], printf_buffer[12], printf_buffer[13], printf_buffer[14], printf_buffer[15], printf_buffer[16], printf_buffer[17], printf_buffer[18], printf_buffer[19], printf_buffer[20], printf_buffer[21], printf_buffer[22], printf_buffer[23], printf_buffer[24], printf_buffer[25], printf_buffer[26], printf_buffer[27], printf_buffer[28], printf_buffer[29], printf_buffer[30], printf_buffer[31]);
    // printf("B_shared_warp_[32:64] : \n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n\n", printf_buffer[32], printf_buffer[33], printf_buffer[34], printf_buffer[35], printf_buffer[36], printf_buffer[37], printf_buffer[38], printf_buffer[39], printf_buffer[40], printf_buffer[41], printf_buffer[42], printf_buffer[43], printf_buffer[44], printf_buffer[45], printf_buffer[46], printf_buffer[47], printf_buffer[48], printf_buffer[49], printf_buffer[50], printf_buffer[51], printf_buffer[52], printf_buffer[53], printf_buffer[54], printf_buffer[55], printf_buffer[56], printf_buffer[57], printf_buffer[58], printf_buffer[59], printf_buffer[60], printf_buffer[61], printf_buffer[62], printf_buffer[63]);
  // }
...
#pragma unroll
    for (int iter_k = 0; iter_k < SHARED_K_ITERS; ++iter_k)
    {
      ...
      // if (blockIdx.x == 0 && threadIdx.x == 0 && threadIdx.y == 0){
      //   // A
      //   // printf("blockIdx.x : %d, threadIdx.x : %d, threadIdx.y : %d, k_0_0 : %d, k_0_0_ld : %d, gemm_iters : %d, iter_k : %d\nA_shared_warp_[:16] : \n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n%.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n\n", blockIdx.x, threadIdx.x, threadIdx.y, k_0_0, k_0_0_ld, gemm_iters, iter_k, __half2float(*(A_shared_warp_ptr+0)), __half2float(*(A_shared_warp_ptr+1)), __half2float(*(A_shared_warp_ptr+2)), __half2float(*(A_shared_warp_ptr+3)), __half2float(*(A_shared_warp_ptr+4)), __half2float(*(A_shared_warp_ptr+5)), __half2float(*(A_shared_warp_ptr+6)), __half2float(*(A_shared_warp_ptr+7)), __half2float(*(A_shared_warp_ptr+8)), __half2float(*(A_shared_warp_ptr+9)), __half2float(*(A_shared_warp_ptr+10)), __half2float(*(A_shared_warp_ptr+11)), __half2float(*(A_shared_warp_ptr+12)), __half2float(*(A_shared_warp_ptr+13)), __half2float(*(A_shared_warp_ptr+14)), __half2float(*(A_shared_warp_ptr+15)));
      //   // blockIdx.x, threadIdx.x, threadIdx.y,
      //   // printf("blockIdx.x : %d, threadIdx.x : %d, threadIdx.y : %d, k_0_0 : %d, k_0_0_ld : %d, gemm_iters : %d, iter_k : %d\nA_shared_warp_[:16] : %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n\n", blockIdx.x, threadIdx.x, threadIdx.y, k_0_0, k_0_0_ld, gemm_iters, iter_k, __half2float(*(A_shared_warp_ptr+0)), __half2float(*(A_shared_warp_ptr+1)), __half2float(*(A_shared_warp_ptr+2)), __half2float(*(A_shared_warp_ptr+3)), __half2float(*(A_shared_warp_ptr+4)), __half2float(*(A_shared_warp_ptr+5)), __half2float(*(A_shared_warp_ptr+6)), __half2float(*(A_shared_warp_ptr+7)), __half2float(*(A_shared_warp_ptr+8)), __half2float(*(A_shared_warp_ptr+9)), __half2float(*(A_shared_warp_ptr+10)), __half2float(*(A_shared_warp_ptr+11)), __half2float(*(A_shared_warp_ptr+12)), __half2float(*(A_shared_warp_ptr+13)), __half2float(*(A_shared_warp_ptr+14)), __half2float(*(A_shared_warp_ptr+15)));
      //   printf("blockIdx.x : %d, threadIdx.x : %d, threadIdx.y : %d, k_0_0 : %d, k_0_0_ld : %d, gemm_iters : %d, iter_k : %d\n", blockIdx.x, threadIdx.x, threadIdx.y, k_0_0, k_0_0_ld, gemm_iters, iter_k);
      //   float printf_buffer[64];
      //   for (int i=0; i < 64; ++i){
      //     printf_buffer[i] = __half2float(*(B_shared_warp_ptr+i));
      //   }
      ...
      for (int i_0_3 = 0; i_0_3 < WARP_M / INTRIN_M; ++i_0_3) // 1
      {
        for (int j_0_4 = 0; j_0_4 < WARP_N / INTRIN_N; ++j_0_4) // 2
        {
      ...
      if (iter_k < WARP_K / INTRIN_K - 1) // iter_k < 3
      {
        if constexpr (STAGES == 1)
          __syncthreads();
      ...
      if (iter_k == WARP_K / INTRIN_K - 2) // iter_k == 2
      {
...
torch::Tensor gemm_4bit_qeft(
    torch::Tensor _in_feats,
    torch::Tensor _kernel,
    torch::Tensor _scales,
    torch::Tensor _zeros,
    torch::Tensor _oweights) // added
{
  ...
  auto oweights = reinterpret_cast<half *>(_oweights.data_ptr<at::Half>()); // added
  auto options =
  ...
  else
  {
    ...
    // 0523 TODO : Implement T2 version QEFT
