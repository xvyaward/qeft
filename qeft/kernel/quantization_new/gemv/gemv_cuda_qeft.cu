// Modified from llm-awq/awq/kernels/csrc/quantization_new/gemv/gemv_cuda.cu
...
__global__ void gemv_kernel_qeft(
  const half* inputs, const uint32_t* weight, const half* scales, const half* zeros, const half* oweight, half* outputs, 
  const int IC, const int OC, const int NumWC)
{
    ...
    // added for weak columns
    int cur_ic_offset = act_k_offset;
    const int wc_bound = (IC - NumWC) / 32 * 32;
    const int wc_offset = NumWC % 32;
    ...
    for (int kk = threadIdx.x * kElemsPerThread; kk < IC * kInterleave; kk += BlockSize * kElemsPerThread)
    {
        ...
        for (int idx = 0; idx < NPerBlock; ++idx)
        {
            ...
            *((float4*)(local_qweights)) = 
            ...
            #pragma unroll
            for (int i = 0; i < kShuffleContinous; ++i) // 4
            {
                #pragma unroll
                for (int j = 0; j < kShuffleStrided; ++j) // 4
                {
                    half2 w = 
                        *reinterpret_cast<half2*>(
                          half_weight_buffer + (i + j * kShuffleContinous)* kShuffleBasicTile // 0 8 16 24 | 2 10 18 26 | 4 12 20 28 | 6 14 22 30
                        );
        ...
        // added for qeft
        // k=128 fast
        if (cur_ic_offset >= wc_bound){
            const half* oweight_ptr = oweight + (blk_row_offset / 2 + thd_row_offset) * NumWC * 2 + (NumWC - (IC - cur_ic_offset) + wc_offset) * 2;
            #pragma unroll
            for (int i = 0; i < 8; ++i){
                *((float4*)(dequantized_weight + i * 8)) = *((float4*)(oweight_ptr + i * 8));
            }
        }
        ...
        cur_ic_offset += act_forward_step;
...
__global__ void gemv_kernel_qeft_perchannel(
  const half* inputs, const uint32_t* weight, const half* scales, const half* zeros, const half* oweight, half* outputs, 
  const int IC, const int OC, const int NumWC)
{
    const int kStride = 64;
    ...
    int group_size)
{
    ...
    auto oweight = reinterpret_cast<half*>(_oweight.data_ptr<at::Half>());
    int numwc = _oweight.size(1) / 2; // packed oweight version
    ...
    if (group_size == 128)
    {
      ...
      {
      case 1:
        gemv_kernel_qeft<N_PER_BLOCK, 1, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 2:
        gemv_kernel_qeft<N_PER_BLOCK, 2, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 3:
        gemv_kernel_qeft<N_PER_BLOCK, 3, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 4:
        gemv_kernel_qeft<N_PER_BLOCK, 4, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 5:
        gemv_kernel_qeft<N_PER_BLOCK, 5, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 6:
        gemv_kernel_qeft<N_PER_BLOCK, 6, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 7:
        gemv_kernel_qeft<N_PER_BLOCK, 7, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      default:
    ...
    else // perchannel
    {
      ...
      {
      case 1:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 1, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 2:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 2, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 3:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 3, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 4:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 4, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 5:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 5, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 6:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 6, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      case 7:
        gemv_kernel_qeft_perchannel<N_PER_BLOCK, 7, BLOCK_SIZE><<<num_blocks, num_threads>>>(
          in_feats, kernel, scaling_factors, zeros, oweight, out_feats, k, n, numwc
        );
        break;
      default:
